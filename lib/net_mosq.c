...
const QUIC_BUFFER Alpn2 = { sizeof("sample") - 1, (uint8_t*)"sample" };
bool Connected = false; // TODO: move for each connection
//HQUIC Registration;
//HQUIC Configuration;
    ...
    )
{
    ...
    //
    if (QUIC_FAILED(Status = MsQuic->StreamOpen(mosq->Connection, QUIC_STREAM_OPEN_FLAG_NONE, ClientStreamCallback, NULL, &Stream))) {
    ...
    //
    //SendBufferRaw = (uint8_t*)malloc(sizeof(QUIC_BUFFER) + SendBuffer);
	SendBufferRaw = (uint8_t*)malloc(sizeof(QUIC_BUFFER));
    ...
    )
{
    ...
    if (QUIC_FAILED(Status = MsQuic->ConfigurationOpen(Registration, &Alpn2, 1, &Settings_, sizeof(Settings_), NULL, &Configuration))) {
...
//
int
ClientConnect(const char *host, uint16_t port, HQUIC* Connection)
{
    //
    // Load the client configuration based on the "unsecure" command line option.
    // TODO: change to secure flag
	fprintf(stderr, "9-2-1\n");
    if (!ClientLoadConfiguration(1)) {
    ...
    if (QUIC_FAILED(Status = MsQuic->ConnectionStart(*Connection, Configuration, QUIC_ADDRESS_FAMILY_UNSPEC, host, port))) {
...
//
void
ClientConnect_listener(struct mosquitto__listener *listener)
{
    //
    // Load the client configuration based on the "unsecure" command line option.
    // TODO: change to secure flag
    ...
    // Start the connection to the server.
    //
    //if (QUIC_FAILED(Status = MsQuic->ConnectionStart(Connection, Configuration, QUIC_ADDRESS_FAMILY_UNSPEC, listener->host, listener->port))) {
    if (QUIC_FAILED(Status = MsQuic->ConnectionStart(Connection, Configuration, QUIC_ADDRESS_FAMILY_UNSPEC, NULL, NULL))) {
...
/*
static int net__init_quic(){
	QUIC_STATUS Status;
	fprintf(stderr, "9-2-0\n");
    //
...
static int net__try_connect_quic(const char *host, uint16_t port, HQUIC *Connection, const char *bind_address, bool blocking)
{
	...
	return ClientConnect(host, port, Connection);
}
...
int net__socket_connect(struct mosquitto *mosq, const char *host, uint16_t port, const char *bind_address, bool blocking)
{
	...
	fprintf(stderr, "9-2 ======\n");
#ifdef WITH_QUIC
	rc = net__try_connect_quic(host, port, &mosq->Connection, bind_address, blocking);
...
#  endif
#else
	return ClientSend(mosq, buf, count);
#endif
